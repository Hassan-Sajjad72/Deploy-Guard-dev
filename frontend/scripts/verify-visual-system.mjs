...
const adminLayout = readFileSync(new URL("../src/components/layout/AdminLayout.jsx", import.meta.url), "utf8");
...
assert.match(layout, /inert=\{navigationOpen \? "" : undefined\}/, "obscured application content must be inert while mobile navigation is open");
assert.match(adminLayout, /href="#admin-main-content"/);
assert.match(adminLayout, /id="admin-main-content"/);
assert.match(sidebar, /useDialogFocus\(onClose, \{ active: isOpen \}\)/, "mobile navigation reuses the focus trap and restoration primitive");
...
assert.match(primitives, /\["ArrowLeft", "ArrowRight", "Home", "End"\]\.includes\(event\.key\)/);
assert.match(primitives, /event\.key === "Home"/);
assert.match(primitives, /event\.key === "End"/);
assert.match(primitives, /event\.key === "ArrowRight"/);
