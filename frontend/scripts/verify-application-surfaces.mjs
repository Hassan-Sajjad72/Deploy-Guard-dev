...
for (const retired of ["detection", "preflight", "environment", "env", "requirements/*"]) {
  assert.equal(routes.includes(`LegacyProjectRedirect />} path="/projects/:projectId/${retired}"`), true, `${retired} redirects to a canonical surface`);
}
assert.doesNotMatch(routes, /ProjectLogs|ProjectEnvVars|ProjectDetection|ProjectPreflight/);
...
assert.match(infrastructure, /Export Terraform ZIP/);
