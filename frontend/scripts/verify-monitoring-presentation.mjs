...
assert.match(sidebar, /label: "Monitoring"[\s\S]*state\?\.runtime\?\.state === "present"/);
...
assert.equal((page.match(/<MetricCard/g) || []).length, 12, "Each of the present and removed runtime views has exactly six service-health cards.");
for (const label of ["Application", "ECS", "Load Balancer", "Logs", "Metrics", "Grafana"]) assert.match(page, new RegExp(`label="${label}"`));
...
assert.match(page, /Configured independently from CloudWatch metrics/);
