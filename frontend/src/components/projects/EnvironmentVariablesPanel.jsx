...
import { ViewportPortal } from "../common/DesignSystem.jsx";
...
export default function EnvironmentVariablesPanel({ projectId, serviceId, serviceName, canManage, onSaved }) {
  ...
  return <section className="environment-manager panel-flat">
    ...
    {modalOpen ? <ViewportPortal><div className="environment-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !busy && setModalOpen(false)}><section aria-modal="true" className="environment-modal" role="dialog"><div className="environment-modal-title"><div><p className="eyebrow">Environment</p><h2>Add an application variable</h2><p>Managed database conflicts are rejected explicitly. Secret values remain masked after save.</p></div><button aria-label="Close" className="subtle-button" disabled={busy} onClick={() => setModalOpen(false)} type="button">Close</button></div><EnvVarForm form={form} isSubmitting={busy} onChange={(event) => { const { checked, name, type, value } = event.target; setForm((current) => ({ ...current, [name]: type === "checkbox" ? checked : value })); }} onSubmit={async (event) => { await submitSingle(event); setModalOpen(false); }} submitLabel="Add variable" /></section></div></ViewportPortal> : null}
  </section>;
}
