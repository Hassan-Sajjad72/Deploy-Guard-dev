...
import { Modal, Tabs } from "../common/DesignSystem.jsx";
...
export default function EnvironmentVariablesPanel({ projectId, serviceId, serviceName, canManage, onSaved }) {
  ...
  return <section className="environment-manager panel-flat">
    <div className="environment-manager-header"><div><p className="eyebrow">Environment{serviceName ? ` · ${serviceName}` : ""}</p><h2>Additional variables</h2><p>These values are injected only into {serviceName || "this service"}. Database aliases are accepted unless they conflict with a managed database attached here; saved secrets are never returned by the API.</p></div>{canManage ? <button className="button" onClick={() => setModalOpen(true)} type="button">Add variable</button> : null}</div>
    {error ? <div className="state error" role="alert">{error}</div> : null}
    {success ? <div aria-live="polite" className="state success" role="status">{success}</div> : null}
    {ignoredEnvironmentNames.map((key) => <div className="state warning" key={key}>{key} is managed by DeployGuard and was ignored.</div>)}
    <Tabs activeId={tab} idPrefix="environment" items={[{ id: "paste", label: "Paste .env" }, { id: "single", label: "Single variable" }]} label="Environment variable entry method" onChange={setTab} />
    {loading ? <p className="muted">Loading environment variables…</p> : null}
    {!loading && tab === "paste" ? <div aria-labelledby="environment-tab-paste" className="environment-paste-panel" id="environment-panel-paste" role="tabpanel" tabIndex={0}><label className="field"><span>Paste KEY=VALUE lines</span><textarea autoComplete="off" name="environmentVariables" onChange={(event) => parsePaste(event.target.value)} placeholder={'APP_BASE_URL=https://example.test\nFEATURE_FLAG=true\nJWT_SECRET="replace-me"'} rows="9" value={paste} /></label>{pasteResult.errors.map((message) => <p className="inline-action-error" key={message}>{message}</p>)}{pasteResult.warnings.map((message) => <p className="environment-warning" key={message}>{message}</p>)}<div className="paste-preview"><strong>{pasteResult.entries.length} valid variable{pasteResult.entries.length === 1 ? "" : "s"}</strong><span>Comments and blank lines are ignored. Duplicate keys are rejected.</span></div>{canManage ? <button className="button" disabled={busy || !pasteResult.entries.length || Boolean(pasteResult.errors.length)} onClick={() => saveBulk(pasteResult.entries, pasteResult.ignoredVariableNames || [])} type="button">{busy ? "Saving…" : "Save pasted variables"}</button> : null}</div> : null}
    {!loading && tab === "single" && canManage ? <div aria-labelledby="environment-tab-single" id="environment-panel-single" role="tabpanel" tabIndex={0}><EnvVarForm form={form} isSubmitting={busy} onCancel={form.id ? () => setForm(emptyForm) : null} onChange={(event) => { const { checked, name, type, value } = event.target; setForm((current) => ({ ...current, [name]: type === "checkbox" ? checked : value })); }} onSubmit={submitSingle} submitLabel={form.id ? "Update variable" : "Add variable"} /></div> : null}
    ...
    {modalOpen ? <Modal className="environment-modal" labelledBy="environment-variable-dialog-title" onClose={() => { if (!busy) setModalOpen(false); }}><div className="environment-modal-title"><div><p className="eyebrow">Environment</p><h2 id="environment-variable-dialog-title">Add an application variable</h2><p>Managed database conflicts are rejected explicitly. Secret values remain masked after save.</p></div><button aria-label="Close" className="subtle-button" disabled={busy} onClick={() => setModalOpen(false)} type="button">Close</button></div><EnvVarForm form={form} isSubmitting={busy} onChange={(event) => { const { checked, name, type, value } = event.target; setForm((current) => ({ ...current, [name]: type === "checkbox" ? checked : value })); }} onSubmit={async (event) => { await submitSingle(event); setModalOpen(false); }} submitLabel="Add variable" /></Modal> : null}
  </section>;
}
