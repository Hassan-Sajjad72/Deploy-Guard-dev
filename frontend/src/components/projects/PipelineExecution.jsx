...
export default function PipelineExecution({ canManage = false, currentState, onRefresh, operations = [], projectId }) {
  ...
  return <div className="pipeline-execution" data-pipeline-execution="true">
    <section aria-label="Deployment execution summary" className="pipeline-summary-grid">
      <MetricCard detail={latest ? `Attempt ${latest.attempt}` : "No GitHub Actions operation yet"} label="Current attempt" value={latest ? `Attempt ${latest.attempt}` : "Not started"} />
    ...
    <Card className="pipeline-timeline-card">
      ...
      {stages.length ? <ol aria-label="GitHub Actions workflow stages" className="pipeline-stage-timeline">
      ...
      </ol> : <p className="pipeline-unavailable">Stage timing unavailable. GitHub Actions did not provide step detail for this operation; the operation status and run link remain the available evidence.</p>}
    ...
    <Card className="pipeline-history-card"><div className="pipeline-section-heading"><div><p className="eyebrow">Attempt history</p><h2>Deployment attempts</h2><p>Each row is a persisted GitHub Actions operation. Retry lineage is retained.</p></div></div>
      <DataTable caption="GitHub Actions deployment attempt history" className="responsive-record-table" label="Deployment attempt history"><thead><tr><th>Attempt</th><th>Generation</th><th>Type</th><th>Result</th><th>Commit</th><th>Duration</th><th>Time</th><th aria-label="Details" /></tr></thead><tbody>{operations.map((operation) => <tr key={operation.id}><td data-label="Attempt">Attempt {operation.attempt}{operation.retryOfOperationId ? <small className="pipeline-retry-lineage">Retry</small> : null}</td><td data-label="Generation" title={operation.generationId || ""}>{compactCommit(operation.generationId)}</td><td data-label="Type">{operationType(operation)}</td><td data-label="Result"><StatusChip status={operation.destroyVerificationStatus === "pending" ? "warning" : operation.status}>{resultLabel(operation)}</StatusChip></td><td data-label="Commit" title={operation.commitSha || ""}>{compactCommit(operation.commitSha)}</td><td data-label="Duration">{duration(operation.createdAt, operationEnd(operation))}</td><td data-label="Time">{date(operation.createdAt)}</td><td data-label="Details"><Button onClick={() => setDetails(operation)} tone="ghost">Details</Button></td></tr>)}{!operations.length ? <tr><td colSpan="8">No GitHub Actions deployment has started yet.</td></tr> : null}</tbody></DataTable>
    </Card>

    {details ? <DetailsDrawer labelledBy="pipeline-attempt-details" onClose={() => setDetails(null)} title={`Attempt ${details.attempt} details`}><dl className="pipeline-attempt-details"><div><dt>Generation</dt><dd>{details.generationId || "Unavailable"}</dd></div><div><dt>Type</dt><dd>{operationType(details)}</dd></div><div><dt>Result</dt><dd>{resultLabel(details)}</dd></div><div><dt>Stage</dt><dd>{details.stageLabel || "Unavailable"}</dd></div>{details.destroyVerificationStatus === "pending" ? <div><dt>Destroy verification</dt><dd>{details.destroyVerificationUnresolved?.length ? `Unresolved: ${details.destroyVerificationUnresolved.join(", ")}` : "Read-only verification is pending."}</dd></div> : null}<div><dt>Started</dt><dd>{date(details.createdAt)}</dd></div><div><dt>Completed</dt><dd>{date(operationEnd(details))}</dd></div><div><dt>Commit</dt><dd>{details.commitSha || "Unavailable"}</dd></div><div><dt>GitHub Actions run</dt><dd>{details.workflowRunId || "Unavailable"}</dd></div>{details.workflowUrl ? <div><dt>Evidence</dt><dd><a href={details.workflowUrl} rel="noreferrer" target="_blank">Open GitHub Actions run</a></dd></div> : null}</dl></DetailsDrawer> : null}
  </div>;
}
