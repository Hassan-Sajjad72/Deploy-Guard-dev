...
export const createBillingCheckout = (plan) => apiRequest("/api/billing/checkout", { method: "POST", body: { plan } });
export const createBillingPortal = () => apiRequest("/api/billing/portal", { method: "POST" });
export const getBillingInvoice = (invoiceId) => apiRequest(`/api/billing/invoices/${id(invoiceId)}`, { cache: "no-store" });
export async function downloadBillingInvoice(invoice) { const response = await fetch(`${getApiBaseUrl()}/api/billing/invoices/${id(invoice.id)}/pdf`, { credentials: "include" }); if (!response.ok) { const payload = await response.json().catch(() => ({})); throw new Error(payload.message || "Invoice download failed"); } const blob = await response.blob(); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `${invoice.invoiceNumber}.pdf`; anchor.click(); URL.revokeObjectURL(url); }
