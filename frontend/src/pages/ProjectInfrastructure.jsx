...
function ServiceRuntimeList({ evidence }) {
  ...
  return <Card><div className="infrastructure-section-heading"><div><p className="eyebrow">Deployable services</p><h2>Service runtime health</h2><p>Each service retains its own immutable image, ECS service, load balancer, endpoint, and logs.</p></div></div><div className="infrastructure-support-grid">{persisted.map((service) => { const current = observed.get(service.serviceId); const healthy = current?.ecs?.runningCount === current?.ecs?.desiredCount && current?.alb?.targetHealth?.every((state) => state === "healthy"); return <article key={service.serviceId}><span>{service.serviceName}</span><strong>{healthy ? "Healthy" : current ? "Attention" : "Unavailable"}</strong><p>{current ? `${current.ecs.runningCount}/${current.ecs.desiredCount} ECS · ${current.alb.targetHealth.join(", ") || "No target sample"}` : "No current AWS observation"}</p>{service.publicUrl ? <a href={service.publicUrl} rel="noreferrer" target="_blank">Open application</a> : null}</article>; })}</div></Card>;
}
...
export default function ProjectInfrastructure() {
  ...
  return <div className="infrastructure-page grid"><PageHeader eyebrow="Infrastructure" title={runtimeTitle} status={infrastructure?.status || "unavailable"} description="Developer-facing health and persisted release evidence for the authoritative runtime generation." />{error ? <ErrorState message={error} onRetry={() => void load()} /> : null}<section aria-label="Infrastructure summary" className="infrastructure-summary-grid"><MetricCard label="Application" value={runtimePresent ? "Healthy" : label(state?.stateAuthority?.runtime?.state)} tone={runtimePresent ? "success" : "neutral"} detail={runtimePresent ? state?.stableUrl || "Verified runtime present" : "No current application runtime"} /><MetricCard label="ECS" value={evidence?.ecs ? `${evidence.ecs.runningCount}/${evidence.ecs.desiredCount}` : label(state?.stateAuthority?.runtime?.state)} tone={evidence?.ecs?.runningCount === evidence?.ecs?.desiredCount ? "success" : "neutral"} detail={evidence?.ecs ? `${evidence.ecs.pendingCount} pending` : "No current AWS observation"} /><MetricCard label="Load balancer" value={evidence?.alb?.targetHealth?.length ? evidence.alb.targetHealth.map(label).join(", ") : label(state?.stateAuthority?.runtime?.state)} tone={evidence?.alb?.targetHealth?.length && evidence.alb.targetHealth.every((item) => item === "healthy") ? "success" : "neutral"} detail={date(evidence?.lastUpdatedAt)} /><MetricCard label="Region" value={evidence?.region || "Unavailable"} detail="Authoritative runtime region" /></section><ServiceFlow evidence={evidence} state={state} /><ServiceRuntimeList evidence={evidence} /><SupportingServices evidence={evidence} /><Pricing cost={evidence?.cost} /><TechnicalDetails evidence={evidence} state={state} /></div>;
}
