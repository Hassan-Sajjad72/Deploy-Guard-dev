...
function aiResultModeLabel(resultMode) { return resultMode === "live" ? "Evidence-based explanation" : "Evidence-only explanation"; }
...
export default function ProjectTroubleshooting() {
  ...
  const currentProjectState = projectStatePresentation(currentState).state;
  ...
  if (!eligibleOperations.length && !selected && !error) return <div className="workspace-page troubleshooting-page troubleshooting-empty-page">
    <PageHeader description="Analyze failed deployments or current LIVE application evidence." eyebrow="Diagnostics" status={currentProjectState} title="Troubleshooting" />
  ...
  return <div className="workspace-page troubleshooting-page">
    ...
    {operation ? <Card><div className="compact-section-heading"><div><p className="eyebrow">Selected operation</p><h2>{label(operation.action)} failed</h2></div><StatusChip status="failed">Failed</StatusChip></div><div className="troubleshooting-operation-grid"><article><span>Project</span><strong>{selected?.evidence?.context?.project?.name || "Current project"}</strong></article><article><span>Service</span><strong>{operation.diagnosis?.serviceName || operation.failureServiceName || selected?.evidence?.context?.runtimeServiceId || "Project operation"}</strong></article><article><span>Deterministic owner</span><strong>{label(operation.diagnosis?.failureOwner || operation.failureOwner || selected?.evidence?.context?.failureOwner || "UNVERIFIED")}{(operation.diagnosis?.externalProvider || operation.externalProvider || selected?.evidence?.context?.externalProvider) ? ` — ${label(operation.diagnosis?.externalProvider || operation.externalProvider || selected?.evidence?.context?.externalProvider)}` : ""}</strong></article><article><span>Pipeline failure code</span><strong>{operation.diagnosis?.terminalFailureCode || operation.failureCode || selected?.evidence?.context?.failureCode || "Not applicable"}</strong></article>{operation.diagnosis ? <article><span>Root cause</span><strong>{operation.diagnosis.rootCauseCode}</strong></article> : null}{operation.diagnosis ? <article><span>Retry decision</span><strong>{label(operation.diagnosis.retryDecision)}</strong></article> : null}<article><span>Action</span><strong>{label(operation.action)}</strong></article><article><span>Commit</span><strong>{String(operation.commitSha || "Unavailable").slice(0, 12)}</strong></article><article><span>Generation</span><strong>{generation(operation.generationId)}</strong></article><article><span>Observed stage</span><strong>{operation.failedStageLabel || label(operation.failedStage)}</strong></article><article><span>Timestamp</span><strong>{date(operationTimestamp(operation))}</strong></article></div><p className="troubleshooting-failure-summary">{operation.diagnosis?.summary || operation.summary || result?.summary || "The selected operation has bounded evidence available for diagnosis."}</p>{operation.diagnosis ? <p className="state warning"><strong>Recovery action:</strong> {operation.diagnosis.recommendedAction}</p> : null}</Card> : null}

    {result ? <Card className="troubleshooting-diagnosis"><div className="compact-section-heading"><div><p className="eyebrow">AI troubleshooting</p><h2>{result.summary}</h2></div><StatusChip status={result.resultMode === "live" ? "healthy" : "warning"}>{aiResultModeLabel(result.resultMode)}</StatusChip></div><section><h3>Likely responsibility</h3><p><strong>{label(details.likelyResponsibility || "INSUFFICIENT_EVIDENCE")}</strong> — AI explanation only. DeployGuard's persisted deterministic diagnosis above remains authoritative.</p></section><section><h3>What happened</h3><p>{result.technicalDetails}</p></section>{details.completedStages?.length ? <section><h3>What DeployGuard successfully completed</h3><ol className="remediation-list">{details.completedStages.map((stage, index) => <li key={`${stage.stage}-${index}`}>{label(stage.stage)}</li>)}</ol></section> : null}<section><h3>Root cause</h3><p>{result.rootCause}</p><p className="muted">Affected component: {details.affectedComponent || "Insufficient evidence"}</p></section><section><h3>Recommended fix</h3><p>{details.recommendedAction}</p><ol className="remediation-list">{result.remediationSteps.map((step, index) => <li key={`${index}-${step}`}>{step}</li>)}</ol></section><section><h3>Retry recommendation</h3><p><strong>{label(details.retryRecommendation?.decision || "INSUFFICIENT_EVIDENCE")}</strong> — {details.retryRecommendation?.reason || "No evidence-based retry recommendation is available."}</p></section><p className="muted">Confidence {Math.round(Number(result.confidence) * 100)}% · {result.limitations}</p><section><h3>Suggested questions</h3><div className="quick-actions">{questions.map((question) => <button className="subtle-button" key={question.type} onClick={() => { setMessage(question.label); setQuestionType(question.type); }} type="button">{question.label}</button>)}</div></section>{result.evidenceReferences?.length ? <div><h3>Evidence references</h3><div className="evidence-reference-list">{result.evidenceReferences.map((reference, index) => <Link className="subtle-button" key={`${reference.eventId || reference.source}-${index}`} to={`/projects/${projectId}/pipeline`}>{sourceLabels[reference.source] || label(reference.source)} · {label(reference.stage)}</Link>)}</div></div> : null}</Card> : null}
