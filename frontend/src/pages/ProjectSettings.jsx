...
export default function ProjectSettings() {
  ...
  return <div className="workspace-page project-settings-page">
    ...
    {activeSection === "source" ? <form aria-labelledby="project-settings-tab-source" className="panel-flat operational-surface settings-simple-form settings-section-panel" id="project-settings-panel-source" onSubmit={saveRepository} role="tabpanel" tabIndex={0}><div><p className="eyebrow">Source</p><h2>Repository and branch</h2><p className="muted">Private repository access uses your encrypted credential.</p></div><label className="field"><span>Repository URL</span><input disabled={!project.canManage || busy} name="repositoryUrl" onChange={change} value={form.repositoryUrl} /></label>{project.canManage ? <button className="secondary-button" disabled={busy} type="submit">Save repository</button> : null}<label className="field"><span>Deployment branch</span>{branches.length ? <select disabled={!project.canManage || busy} name="targetBranch" onChange={change} value={form.targetBranch}>{branches.map((branch) => <option key={branch} value={branch}>{branch}</option>)}</select> : <input disabled={!project.canManage || busy} name="targetBranch" onChange={change} value={form.targetBranch} />}</label>{project.canManage ? <div className="quick-actions"><button className="subtle-button" disabled={busy} onClick={loadBranches} type="button">Load branches</button><button className="secondary-button" disabled={busy} onClick={saveBranch} type="button">Save branch</button></div> : null}</form> : null}

    {activeSection === "services" ? <section aria-labelledby="project-settings-tab-services" className="panel-flat operational-surface settings-section-panel" id="project-settings-panel-services" role="tabpanel" tabIndex={0}><div className="compact-section-heading"><div><p className="eyebrow">Services</p><h2>Deployable applications</h2><p className="muted">Each service directory is configured explicitly; its application port is detected automatically.</p></div>{project.canManage ? <button className="secondary-button" disabled={busy} onClick={addService} type="button">+ Add service</button> : null}</div>{services.length > 1 ? <form className="settings-entrypoint-form" onSubmit={saveApplicationService}><label className="field"><span>Open Application service</span><select disabled={!project.canManage || busy} name="applicationEntryPointServiceId" onChange={(event) => setApplicationEntryPointServiceId(event.target.value)} required value={applicationEntryPointServiceId}><option value="">Choose a service</option>{services.map((service) => <option key={service.id} value={service.id}>{service.name} — {service.serviceDirectory}</option>)}</select></label>{project.canManage ? <button className="secondary-button" disabled={busy || !applicationEntryPointServiceId} type="submit">Save entrypoint</button> : null}</form> : <p className="muted">{services[0]?.name || "The only service"} is the application entrypoint.</p>}<div className="settings-service-list">{services.map((service) => <details className="settings-service-card" key={service.id}><summary><span><strong>{service.name}</strong><small>{service.serviceDirectory} · {service.servicePort ? `Detected port ${service.servicePort}` : "Port detected at deployment"}</small></span><span>Edit</span></summary><div className="new-project-fields"><label className="field"><span>Name</span><input autoComplete="off" disabled={!project.canManage || busy} name="serviceName" onChange={(event) => setServices((current) => current.map((item) => item.id === service.id ? { ...item, name: event.target.value } : item))} value={service.name} /></label><label className="field"><span>Directory</span><input autoComplete="off" disabled={!project.canManage || busy} name="serviceDirectory" onChange={(event) => setServices((current) => current.map((item) => item.id === service.id ? { ...item, serviceDirectory: event.target.value } : item))} value={service.serviceDirectory} /></label></div>{project.canManage ? <div className="quick-actions"><button className="button" disabled={busy} onClick={() => saveService(service)} type="button">Save service</button>{services.length > 1 ? <button className="danger-text-button" disabled={busy} onClick={() => removeService(service)} type="button">Remove</button> : null}</div> : null}</details>)}</div></section> : null}

    {activeSection === "variables" ? <section aria-labelledby="project-settings-tab-variables" className="panel-flat operational-surface settings-section-panel" id="project-settings-panel-variables" role="tabpanel" tabIndex={0}><div className="compact-section-heading"><div><p className="eyebrow">Variables</p><h2>Service environment</h2><p className="muted">Values are scoped to one service and remain masked after save.</p></div></div>{services.length > 1 ? <label className="field settings-service-selector"><span>Service</span><select name="selectedServiceId" onChange={(event) => setSelectedServiceId(event.target.value)} value={selectedService?.id || ""}>{services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select></label> : null}{selectedService ? <EnvironmentVariablesPanel canManage={Boolean(project.canManage)} projectId={projectId} serviceId={selectedService.id} serviceName={selectedService.name} /> : <p className="muted">Add a service before configuring variables.</p>}</section> : null}

    {activeSection === "database" ? <form aria-labelledby="project-settings-tab-database" className="panel-flat operational-surface settings-simple-form settings-section-panel" id="project-settings-panel-database" onSubmit={saveDatabase} role="tabpanel" tabIndex={0}><div><p className="eyebrow">Database</p><h2>Managed database</h2><p className="muted">Configure one managed database and choose its attached service.</p></div><label className="field"><span>Database type</span><select disabled={!project.canManage || busy} onChange={(event) => setDatabase((current) => event.target.value === "none" ? { ...current, provider: "none" } : { ...current, provider: "managed", engine: event.target.value })} value={database.provider === "managed" ? database.engine : "none"}><option value="none">Managed ENV Database</option><option value="postgres">Managed Database — PostgreSQL</option><option value="mysql">Managed Database — MySQL</option><option value="mongodb">Managed Database — MongoDB</option></select></label>{database.provider === "managed" ? <><label className="field"><span>Attached service</span><select disabled={!project.canManage || busy} onChange={(event) => setDatabase((current) => ({ ...current, attachedServiceId: event.target.value }))} value={database.attachedServiceId}>{services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select></label><label className="checkbox-row"><input checked={database.persistenceEnabled} disabled={!project.canManage || busy} onChange={(event) => setDatabase((current) => ({ ...current, persistenceEnabled: event.target.checked }))} type="checkbox" />Persist database data</label></> : null}{project.canManage ? <button className="button" disabled={busy} type="submit">Save database</button> : null}</form> : null}
