...
export default function ProjectSettings() {
  ...
  async function saveService(service) { await action(async () => { const response = await updateProjectService(projectId, service.id, { name: service.name, serviceDirectory: service.serviceDirectory, servicePort: Number(service.servicePort || 8080) }); setServices((current) => current.map((item) => item.id === service.id ? response.service : item)); }, `${service.name} saved.`); }
  ...
  return <div className="workspace-page">
    ...
    <section className="panel-flat"><div className="compact-section-heading"><div><p className="eyebrow">Services</p><h2>Deployable applications</h2><p className="muted">Each service is selected explicitly. Railpack owns its build and start behavior.</p></div>{project.canManage ? <button className="secondary-button" disabled={busy} onClick={addService} type="button">+ Add Service</button> : null}</div>{services.length > 1 ? <form className="settings-simple-form" onSubmit={saveApplicationService}><label className="field"><span>Application service</span><select disabled={!project.canManage || busy} onChange={(event) => setApplicationEntryPointServiceId(event.target.value)} required value={applicationEntryPointServiceId}><option value="">Choose the service Open Application should open</option>{services.map((service) => <option key={service.id} value={service.id}>{service.name} — {service.serviceDirectory}</option>)}</select></label>{project.canManage ? <button className="secondary-button" disabled={busy || !applicationEntryPointServiceId} type="submit">Save application service</button> : null}</form> : <p className="muted">{services[0]?.name || "The only service"} is the application service.</p>}{services.map((service) => <article className="settings-service-card" key={service.id}><div className="new-project-fields"><label className="field"><span>Name</span><input disabled={!project.canManage || busy} onChange={(event) => setServices((current) => current.map((item) => item.id === service.id ? { ...item, name: event.target.value } : item))} value={service.name} /></label><label className="field"><span>Repository-relative directory</span><input disabled={!project.canManage || busy} onChange={(event) => setServices((current) => current.map((item) => item.id === service.id ? { ...item, serviceDirectory: event.target.value } : item))} value={service.serviceDirectory} /></label><label className="field"><span>Application Port</span><input disabled={!project.canManage || busy} max="65535" min="1" onChange={(event) => setServices((current) => current.map((item) => item.id === service.id ? { ...item, servicePort: event.target.value } : item))} type="number" value={service.servicePort || 8080} /><small>Port this application listens on. Leave 8080 unless your application uses another port.</small></label></div>{project.canManage ? <div className="quick-actions"><button className="secondary-button" disabled={busy || !Number.isInteger(Number(service.servicePort || 8080)) || Number(service.servicePort || 8080) < 1 || Number(service.servicePort || 8080) > 65535} onClick={() => saveService(service)} type="button">Save service</button>{services.length > 1 ? <button className="danger-text-button" disabled={busy} onClick={() => removeService(service)} type="button">Remove</button> : null}</div> : null}<EnvironmentVariablesPanel canManage={Boolean(project.canManage)} projectId={projectId} serviceId={service.id} serviceName={service.name} /></article>)}</section>
    <form className="panel-flat settings-simple-form" onSubmit={saveDatabase}><div><p className="eyebrow">Data</p><h2>Database</h2><p className="muted">Use existing ENV for an external database, or let DeployGuard manage one persistent database for one service.</p></div><label className="field"><span>Database</span><select disabled={!project.canManage || busy} onChange={(event) => setDatabase((current) => event.target.value === "none" ? { ...current, provider: "none" } : { ...current, provider: "managed", engine: event.target.value })} value={database.provider === "managed" ? database.engine : "none"}><option value="none">No managed database / use existing ENV</option><option value="postgres">PostgreSQL</option><option value="mysql">MySQL</option><option value="mongodb">MongoDB</option></select></label>{database.provider === "managed" ? <><label className="field"><span>Attach database to</span><select disabled={!project.canManage || busy} onChange={(event) => setDatabase((current) => ({ ...current, attachedServiceId: event.target.value }))} value={database.attachedServiceId}>{services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select></label><label className="checkbox-row"><input checked={database.persistenceEnabled} disabled={!project.canManage || busy} onChange={(event) => setDatabase((current) => ({ ...current, persistenceEnabled: event.target.checked }))} type="checkbox" />Persist database data</label></> : null}{project.canManage ? <button className="button" disabled={busy} type="submit">Save database settings</button> : null}</form>
