...
export default function ProjectSettings() {
  ...
  return <div className="workspace-page project-settings-page">
    ...
    {activeSection === "database" ? <form aria-labelledby="project-settings-tab-database" className="panel-flat operational-surface settings-simple-form settings-section-panel" id="project-settings-panel-database" onSubmit={saveDatabase} role="tabpanel" tabIndex={0}><div><p className="eyebrow">Database</p><h2>Managed database</h2><p className="muted">Configure one managed database and choose its attached service.</p></div><label className="field"><span>Database type</span><select disabled={!project.canManage || busy} onChange={(event) => setDatabase((current) => event.target.value === "none" ? { ...current, provider: "none" } : { ...current, provider: "managed", engine: event.target.value })} value={database.provider === "managed" ? database.engine : "none"}><option value="none">No managed database / use existing ENV</option><option value="postgres">PostgreSQL</option><option value="mysql">MySQL</option><option value="mongodb">MongoDB</option></select></label>{database.provider === "managed" ? <><label className="field"><span>Attached service</span><select disabled={!project.canManage || busy} onChange={(event) => setDatabase((current) => ({ ...current, attachedServiceId: event.target.value }))} value={database.attachedServiceId}>{services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select></label><label className="checkbox-row"><input checked={database.persistenceEnabled} disabled={!project.canManage || busy} onChange={(event) => setDatabase((current) => ({ ...current, persistenceEnabled: event.target.checked }))} type="checkbox" />Persist database data</label></> : null}{project.canManage ? <button className="button" disabled={busy} type="submit">Save database</button> : null}</form> : null}
