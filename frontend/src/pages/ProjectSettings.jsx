...
export default function ProjectSettings() {
  ...
  return <div className="workspace-page project-settings-page">
    ...
    {activeSection === "services" ? <section aria-labelledby="project-settings-tab-services" className="panel-flat operational-surface settings-section-panel" id="project-settings-panel-services" role="tabpanel"><div className="compact-section-heading"><div><p className="eyebrow">Services</p><h2>Deployable applications</h2><p className="muted">Each service is configured explicitly.</p></div>{project.canManage ? <button className="secondary-button" disabled={busy} onClick={addService} type="button">+ Add service</button> : null}</div>{services.length > 1 ? <form className="settings-entrypoint-form" onSubmit={saveApplicationService}><label className="field"><span>Open Application service</span><select disabled={!project.canManage || busy} onChange={(event) => setApplicationEntryPointServiceId(event.target.value)} required value={applicationEntryPointServiceId}><option value="">Choose a service</option>{services.map((service) => <option key={service.id} value={service.id}>{service.name} — {service.serviceDirectory}</option>)}</select></label>{project.canManage ? <button className="secondary-button" disabled={busy || !applicationEntryPointServiceId} type="submit">Save entrypoint</button> : null}</form> : <p className="muted">{services[0]?.name || "The only service"} is the application entrypoint.</p>}<div className="settings-service-list">{services.map((service) => <details className="settings-service-card" key={service.id}><summary><span><strong>{service.name}</strong><small>{service.serviceDirectory} · Port {service.servicePort || 8080}</small></span><span>Edit</span></summary><div className="new-project-fields"><label className="field"><span>Name</span><input disabled={!project.canManage || busy} onChange={(event) => setServices((current) => current.map((item) => item.id === service.id ? { ...item, name: event.target.value } : item))} value={service.name} /></label><label className="field"><span>Directory</span><input disabled={!project.canManage || busy} onChange={(event) => setServices((current) => current.map((item) => item.id === service.id ? { ...item, serviceDirectory: event.target.value } : item))} value={service.serviceDirectory} /></label><label className="field"><span>Application port</span><input disabled={!project.canManage || busy} max="65535" min="1" onChange={(event) => setServices((current) => current.map((item) => item.id === service.id ? { ...item, servicePort: event.target.value } : item))} type="number" value={service.servicePort || 8080} /><small>Port your application listens on. DeployGuard manages PORT and HOST automatically.</small></label></div>{project.canManage ? <div className="quick-actions"><button className="button" disabled={busy || !Number.isInteger(Number(service.servicePort || 8080)) || Number(service.servicePort || 8080) < 1 || Number(service.servicePort || 8080) > 65535} onClick={() => saveService(service)} type="button">Save service</button>{services.length > 1 ? <button className="danger-text-button" disabled={busy} onClick={() => removeService(service)} type="button">Remove</button> : null}</div> : null}</details>)}</div></section> : null}
