...
export const RAILPACK_WORKFLOW_INPUTS = [
  ...
  { name: "platform_port", required: true, type: "string" }, { name: "control_plane_sha", required: true, type: "string" },
...
export type RailpackServiceRuntimeConfiguration = { serviceId: string; serviceName: string; serviceDirectory: string; runtimeConfigRevisionId: string; buildEnvironment: Record<string, string>; buildSecretReferences: Record<string, string>; environment: Record<string, string>; secretReferences: Record<string, string>; databaseAttached: boolean; managedDatabase: { engine: "postgres" | "mysql" | "mongodb" | null; aliases: string[]; secretVersionId?: string | null }; rollbackImage?: string };
export type RailpackRuntimeConfiguration = { schemaVersion: 2; projectId: string; environmentName: string; operationId: string; sourceSha: string; services: RailpackServiceRuntimeConfiguration[]; projectDeletion?: { generationIds: string[] } };
...
export function assertRailpackRuntimeConfiguration(value: RailpackRuntimeConfiguration) {
  if (value.schemaVersion !== 2 || !UUID.test(value.projectId) || !UUID.test(value.operationId) || !SHA.test(value.sourceSha) || !ENVIRONMENT_NAME.test(value.environmentName) || !Array.isArray(value.services) || !value.services.length || value.services.length > 20) throw new Error("Railpack runtime configuration identity is invalid.");
  ...
  for (const service of value.services) {
    ...
    for (const [key, item] of Object.entries(service.buildEnvironment)) if (!KEY.test(key) || typeof item !== "string" || ["PORT", "HOST"].includes(key) || isDeployGuardManagedDatabaseAlias(key)) throw new Error("Railpack build environment is invalid.");
    for (const [key, reference] of Object.entries(service.buildSecretReferences)) if (!KEY.test(key) || !SECRET_VALUE_FROM.test(reference) || ["PORT", "HOST"].includes(key) || isDeployGuardManagedDatabaseAlias(key)) throw new Error("Railpack build secret reference is invalid.");
