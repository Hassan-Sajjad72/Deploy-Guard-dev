...
export const DEPLOYGUARD_DEFAULT_SERVICE_PORT = 8080 as const;

export function effectiveServicePort(value: unknown) {
  if (value === undefined || value === null || value === "") return DEPLOYGUARD_DEFAULT_SERVICE_PORT;
  const port = typeof value === "string" && /^\d+$/.test(value) ? Number(value) : value;
  if (!Number.isInteger(port) || Number(port) < 1 || Number(port) > 65535) throw new Error("Application port must be an integer from 1 to 65535.");
  return Number(port);
}
