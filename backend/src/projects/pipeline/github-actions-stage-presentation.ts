const FRIENDLY_GITHUB_ACTIONS_STAGES: Record<string, string> = {
  ...
  runtime_secret_materialization: "Prepare Runtime Secrets",
...
export function githubActionsFailureLifecyclePhase(failedStage: unknown, action: GithubActionsPresentationAction = "deploy"): "source" | "build" | "deploy" | "verify" | "finalize" {
  ...
  if (["workflow_bootstrap", "set_up_job", "workflow_dispatch", "source_resolution", "service_directory_validation", "service_port_resolution", "deployment_requirement_admission", "managed_database_reconciliation", "caller_reconciliation", "runtime_secret_materialization", "configure_aws_credentials_through_oidc", "checkout_exact_application_source", "validate_immutable_release_input"].includes(key)) return "source";
