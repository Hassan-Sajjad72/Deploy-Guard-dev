...
export function classifyStructuredFailure(stage: string, safeEvidence: string): StructuredFailure {
  ...
  if (["DG_TERRAFORM_APPLY_FAILED", "DG_ECR_PUBLISH_FAILED", "DG_AWS_AUTHORIZATION_FAILED", "DG_AWS_PROVIDER_FAILED", "DG_RUNTIME_SECRET_MATERIALIZATION_FAILED", "DG_CLOUD_MAP_REGISTRATION_TIMEOUT", "DG_ALB_NOT_ACTIVE", "DG_PUBLIC_DNS_UNRESOLVED", "DG_PUBLIC_CONNECTION_FAILED", "DG_PUBLIC_REACHABILITY_FAILED"].includes(code)) return { failureOwner: "EXTERNAL_PROVIDER", externalProvider: "aws", failureCode: code, failureServiceId: serviceId };
  if (code === "DG_ALB_LISTENER_MISMATCH") return { failureOwner: "DEPLOYGUARD_PLATFORM", externalProvider: null, failureCode: code, failureServiceId: serviceId };
