...
export class DockerfileSecurityService {
  analyze(content: string) {
    ...
    lines.forEach((source, index) => {
      ...
      if (/^(?:ARG|ENV)\s+[^=\s]*(?:TOKEN|PASSWORD|SECRET|PRIVATE_KEY|AWS_ACCESS_KEY)/i.test(line)) add("secret_in_dockerfile", "blocking", "Do not define credentials or secrets in Dockerfile ARG/ENV instructions.", index + 1);
