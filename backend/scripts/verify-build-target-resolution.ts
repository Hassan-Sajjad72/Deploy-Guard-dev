...
const packageJson = (name: string, dependencies: Record<string, string> = {}) => JSON.stringify({ name, version: "1.0.0", scripts: { start: "node index.js" }, dependencies });
...
void (async () => {
  ...
  try {
    ...
    const local = join(root, "local-path"); await mkdir(local); await write(local, "service/package.json", packageJson("local-service", { helper: "file:../helper" })); await write(local, "helper/package.json", packageJson("helper")); assert.deepEqual((await resolve(local, "service")).dependencyPaths, ["helper"]);
