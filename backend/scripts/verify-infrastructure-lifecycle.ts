...
service.assertSharedStateBucketNotTracked(JSON.stringify({ resources: [{ type: "aws_ecs_service", instances: [{ attributes: { id: "app" } }] }] }), "deployguard-canonical-state");
assert.throws(() => service.assertSharedStateBucketNotTracked(JSON.stringify({ resources: [{ type: "aws_s3_bucket", instances: [{ attributes: { bucket: "deployguard-canonical-state" } }] }] }), "deployguard-canonical-state"), /shared Terraform state bucket/);
