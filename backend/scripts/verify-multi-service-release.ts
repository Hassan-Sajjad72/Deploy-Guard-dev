...
const runtime: RailpackRuntimeConfiguration = { schemaVersion: 2, projectId, operationId, environmentName: "dev", sourceSha, services: ids.map((serviceId, index) => ({ serviceId, serviceName: index ? "API" : "Web", serviceDirectory: index ? "api" : "web", environment: { PORT: "8080", HOST: "0.0.0.0" }, secretReferences: {}, databaseAttached: false, managedDatabase: { engine: null, aliases: [] } })) };
...
const rollback = service.rollbackTarget(release);
assert.deepEqual(rollback.services.map((item: any) => item.immutableImage), valid.services.map((item: any) => item.image));
assert.doesNotMatch(JSON.stringify(rollback), /localImage|buildCommand|railpack/i);
console.log("MULTI_SERVICE_RELEASE=PASS COMPLETE_SET_REQUIRED=1 IMMUTABLE_ROLLBACK_SET=1 PARTIAL_PROMOTION=0");
