...
function verify(mode: "correct" | "wrong_env" | "wrong_secret") {
  ...
  writeFileSync(outputs, JSON.stringify({ ecs_cluster_name: "cluster", vpc_id: "vpc-1", public_subnet_ids: ["subnet-1"], database: null, services: { [serviceId]: { image, ecs_service_arn: ecsService, ecs_service_name: "application", task_definition_arn: task, alb_target_group_arn: "target-group", cloudwatch_log_group_name: "/deployguard/application", public_url: "http://application.test", security_group_id: "sg-1" } } }), "utf8");
