...
while IFS= read -r service; do
  ...
  expected_secret_names="$(jq -c '[.secretReferences | keys[], (.managedDatabase.aliases[]? | select(test("(PASSWORD|URL|URI)$")))] | unique | sort' <<<"$expected")"
  expected_environment_names="$(jq -c '[.environment | keys[], (.managedDatabase.aliases[]? | select(test("(PASSWORD|URL|URI)$") | not))] | unique | sort' <<<"$expected")"
  ...
  jq -e --arg image "$expected_image" --arg log "$log_group" --argjson secrets "$expected_secret_names" --argjson environment "$expected_environment_names" '
    ...
    and ([$app.secrets[]?.name] | unique | sort) == $secrets and ([$app.environment[]?.name] | unique | sort) == $environment
    and ($app.environment | any(.name == "PORT" and .value == "8080")) and ($app.environment | any(.name == "HOST" and .value == "0.0.0.0"))
  ...
  curl --show-error --silent --retry 20 --retry-delay 3 --retry-connrefused --output /dev/null "$(jq -r '.public_url' <<<"$deployed")" || { echo "DG_FAILURE serviceId=$service_id code=DG_PUBLIC_REACHABILITY_FAILED stage=public_health" >&2; exit 1; }
